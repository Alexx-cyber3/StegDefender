...
class AudioAnalyzer(BaseAnalyzer):
    def analyze(self):
        try:
            ...
            # Add Steghide check for supported audio (WAV/AU)
            self.check_steghide()
    ...
    def check_steghide(self):
        ...
        if not SteghideWrapper.is_available():
            # Only mention if it's a format steghide supports
            ext = os.path.splitext(self.file_path)[1].lower()
            if ext in ['.wav', '.au']:
                self.add_detail("Steghide tool not found. Skipping deep bitstream analysis for audio.", "info")
            return

        is_stego, encrypted, info = SteghideWrapper.get_info(self.file_path)
        if is_stego:
            if encrypted:
                self.add_detail("Steghide container detected in audio (Encrypted).", "danger")
                self.set_verdict("Stego Detected")
            else:
                self.add_detail("Steghide container detected in audio (No passphrase used).", "danger")
                self.set_verdict("Stego Detected")
                data, msg = SteghideWrapper.extract(self.file_path, password="")
                if data:
                    self.save_artifact("steghide_audio_extracted.bin", data, is_stego=True)
