...
class GenericAnalyzer(BaseAnalyzer):
    def analyze(self):
        try:
            ...
            # Limit read for large files to keep it stable
            read_limit = 10 * 1024 * 1024 # 10 MB
            with open(self.file_path, 'rb') as f:
                if file_size > read_limit:
                    data = f.read(read_limit)
                    self.add_detail(f"File is large ({file_size / 1024 / 1024:.2f} MB). Analyzing only the first 10MB.", "info")
                else:
                    data = f.read()
            
            entropy = calculate_entropy(data)
            self.add_detail(f"Sample Entropy: {entropy:.4f}", "info")
