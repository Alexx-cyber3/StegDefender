...
try:
    ...
    from stegdefender.utils.ai_copilot import AICopilot
...
def enrich_artifacts_with_urls(node, parent_artifact_url=None):
    ...
    # Check confirmed_stego_artifacts
    if "confirmed_stego_artifacts" in node:
        for artifact in node["confirmed_stego_artifacts"]:
            full_path = os.path.abspath(artifact["path"])
            if full_path.startswith(extracted_abs):
                rel_path = os.path.relpath(full_path, extracted_abs)
                rel_path = rel_path.replace("\\", "/")
...
@app.get("/ai-summary/{file_id}")
async def get_ai_summary(file_id: str):
    data = analysis_cache.get(file_id)
    if not data:
        # Check history if not in cache
        history_path = os.path.join(HISTORY_DIR, f"{file_id}.json")
        if os.path.exists(history_path):
            with open(history_path, 'r') as f:
                data = json.load(f)
    ...
    copilot = AICopilot()
    summary = copilot.generate_summary(data['results'])
...
@app.post("/extract/steghide")
async def extract_steghide(file_id: str = Form(...), password: str = Form(...)):
    """
    Extracts a password-protected Steghide artifact using its file_id.
    """
    from stegdefender.utils.steghide_wrapper import SteghideWrapper
    from stegdefender.core.extractor import StegoExtractor

    # 1. Locate the file (this assumes we track artifact paths in our cache)
    # Since artifact paths aren't currently in the simple cache, 
    # we need to search our extraction directories for a file matching the ID pattern.
    import glob
    search_pattern = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "stegdefender", "extracted_data", "**", f"*{file_id}*")
    ...
    # 2. Extract using SteghideWrapper
    data, msg = SteghideWrapper.extract(target_path, password=password)
    
    if data:
        # Return the data as a streaming response for download
        import io
        from fastapi.responses import StreamingResponse
        return StreamingResponse(
            io.BytesIO(data),
            media_type="application/octet-stream",
            headers={"Content-Disposition": f"attachment; filename=extracted_{file_id}.bin"}
        )
    else:
